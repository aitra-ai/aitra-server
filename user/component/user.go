...
type UserComponent interface {
	...
	SigninWithPassword(ctx context.Context, username, password string) (*types.JWTClaims, string, error)
...
func (c *userComponentImpl) createFromSSOUser(ctx context.Context, cu *rpc.SSOUserInfo) (*database.User, error) {
	...
	// Auto-grant $10 free credit for new user
	go func() {
		creditStore := database.NewUserCreditStore()
		_ = creditStore.Create(context.Background(), &database.UserCredit{
			UserID:    user.ID,
			Username:  user.Username,
			AmountUSD: 10.0,
			Note:      "Initial free credit",
			GrantedBy: "system",
			CreatedAt: time.Now(),
		})
	}()
...
// SigninWithPassword authenticates a user with username+password via Casdoor password grant,
// syncs the user to csghub DB, and returns a csghub JWT.
func (c *userComponentImpl) SigninWithPassword(ctx context.Context, username, password string) (*types.JWTClaims, string, error) {
	c.lazyInit()

	// Get Casdoor access token via password grant
	accessToken, err := c.sso.GetTokenByPassword(ctx, username, password)
	if err != nil {
		return nil, "", fmt.Errorf("invalid username or password: %w", err)
	}

	// Get user info from Casdoor
	cu, err := c.sso.GetUserInfo(ctx, accessToken)
	if err != nil {
		return nil, "", fmt.Errorf("failed to get user info: %w", err)
	}

	// Find or create user in csghub DB
	exists, err := c.userStore.IsExistByUUID(ctx, cu.UUID)
	if err != nil {
	...
	if !exists {
		...
		go func() {
			...
			defer cancel()
			dbu.LastLoginAt = time.Now().Format("2006-01-02 15:04:05")
			_ = c.userStore.Update(updateCtx, dbu, "")
		}()
	}

	// Generate csghub JWT
	hubToken, signed, err := c.jwtc.GenerateToken(ctx, types.CreateJWTReq{UUID: dbu.UUID})
	if err != nil {
