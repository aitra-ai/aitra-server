...
func (m *openaiComponentImpl) GetModelByID(c context.Context, username, modelID string) (*types.Model, error) {
	...
	if err != nil {
		...
		// redis.Nil means field not found in cache hash - fall through to DB lookup
		slog.Debug("model not in cache hash, querying DB", "modelID", modelID)
	} else if model != nil {
		// Cache hit, return the model
		return model, nil
	} else {
		// cache key expired - fall through to DB lookup
		slog.Debug("cache expired, refresh cache")
	}

	// Cache miss or expired: query available models from DB
	models, err := m.GetAvailableModels(c, username)
	if err != nil {
		return nil, err
	}
